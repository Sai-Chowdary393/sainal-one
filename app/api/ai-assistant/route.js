...
function looksLikeActionRequest(prompt) {
  ...
  if (actionPatterns.some((pattern) => pattern.test(text))) return true;
  if (analysisPatterns.some((pattern) => pattern.test(text))) return false;
  return false;
}

function localDateTime(timezone) {
  try {
    return new Intl.DateTimeFormat("en-GB", { timeZone: timezone, dateStyle: "full", timeStyle: "long", hour12: false }).format(new Date());
  } catch {
    return new Intl.DateTimeFormat("en-GB", { timeZone: "UTC", dateStyle: "full", timeStyle: "long", hour12: false }).format(new Date());
  }
}
...
async function loadInvoicePayments(access, invoices) {
  const invoiceIds = (invoices || []).map((item) => item.id).filter(Boolean);
...
async function loadBusinessData(access) {
  ...
  businessData.invoicePayments = await loadInvoicePayments(access, businessData.invoices);
