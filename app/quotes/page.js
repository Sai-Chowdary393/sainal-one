...
export default function QuotesPage() {
  ...
  async function fetchQuotes() {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || "Failed to load quotes."
        );
      }
      ...
      setErrorMessage(
        error.message || "We could not load the quotes."
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || "Failed to create quote."
        );
      }

      const createdQuote = Array.isArray(data)
        ? data[0]
        : data;
      ...
      alert(
        error.message || "Error creating quote."
      );
    } finally {
      setSaving(false);
    }
  ...
  const sentQuotes = quotes.filter(
    (quote) =>
      normaliseStatus(quote.status) === "sent"
  ).length;

  const acceptedQuotes = quotes.filter(
    (quote) =>
      normaliseStatus(quote.status) === "accepted"
  ).length;

  const pipelineValue = quotes
    .filter(
      (quote) =>
        normaliseStatus(quote.status) !== "rejected" &&
        normaliseStatus(quote.status) !== "expired"
    )
    .reduce(
      (total, quote) =>
        ...
        <div className={styles.page}>
          <section className={styles.pageHeader}>
            <div className={styles.pageHeaderCopy}>
              ...
              <p>
                Prepare and track commercial quotations.
                Contact details and full pricing documents
                remain inside each secure quote workspace.
              </p>
            </div>
            ...
            <section className={styles.formPanel}>
              <div className={styles.formHeading}>
                <div>
                  ...
                  <p>
                    Enter the commercial information and
          ...
          <section className={styles.summaryGrid}>
            ...
            <SummaryCard
              ...
              label="Sent"
              value={sentQuotes}
              detail="Awaiting customer response"
              tone="purple"
            />
            ...
            <section className={styles.tablePanel}>
              <div className={styles.tableHeading}>
                <div>
                  ...
                  <p>
                    Sensitive client contact details and the
                    full quote document are available only
                    inside each quote workspace.
                  </p>
                </div>
  ...
  return (
    <div
      className={`${styles.summaryCard} ${
        styles[`summary${capitalise(tone)}`] || ""
      }`}
    >
