...
export async function planRequest({ openai, prompt, conversation, businessData, employees, timezone, currentTime, localTime, forceActions = false }) {
  ...
  //
  // "Mark quote SNQ-... as Accepted" is a commercial conversion action.
  // Accepted is intentionally NOT written through update_quote; it uses the
  // quote-to-customer flow so the quote/customer relationship stays valid.
  // =====================================================
...
export function buildDeterministicPlan({ prompt, conversation, businessData, timezone }) {
  ...
  if (wantsAcceptQuote) {
    if (
      normalise(
        quoteForWorkflow.status
      ) === "accepted"
    ) {
      return {
        mode: "analysis",
        summary:
          `${quoteForWorkflow.quote_number || "This quote"} is already Accepted.`,
        actions: [],
      };
    }
    ...
    actions.push({
      type:
        "convert_quote_to_customer",
      label:
        `Accept ${quoteForWorkflow.quote_number || "quote"}`,
      reason:
        "Accept the matched quote and ensure it is linked to its customer.",
      data: {
        record_id:
          quoteForWorkflow.id,
      },
