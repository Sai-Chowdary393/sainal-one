...
import { markInvoiceAsPaid, convertQuoteToInvoice } from "../services/invoiceService";
...
export async function planRequest({ openai, prompt, conversation, businessData, employees, timezone, currentTime }) {
...
async function executeOne({ action, access, businessData, permissions, employees, profile, openai, results }) {
  ...
  switch (action.type) {
    ...
    case "mark_invoice_paid": {
      ...
      const result = await markInvoiceAsPaid({ prompt: `Mark invoice ${invoice.invoice_number} paid`, invoices: [invoice], quotes: businessData.quotes, organizationId, employeeId });
      return { message: result.alreadyPaid ? `Invoice already paid: ${invoice.invoice_number}` : `Invoice marked as paid: ${result.invoice.invoice_number}`, data: { invoice: result.invoice } };
    }
    case "send_email": {
