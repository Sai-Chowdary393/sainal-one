...
const ACTIONS = new Set([
  ...
  "create_project_from_customer", "create_quote", "create_proposal",
...
function isClearActionInstruction(prompt) {
  ...
  return Boolean(
    /^(?:please\s+)?(?:create|add|make|update|change|edit|set|mark|convert|start|begin|open|schedule|book|arrange|send|draft|write|compose|prepare|record|assign|reassign|complete|finish|cancel|reopen|reschedule|deactivate|activate)\b/i.test(source) ||
    /\b(?:please|go ahead and|i want you to|i'd like you to|i would like you to)\s+(?:create|add|make|update|change|edit|set|mark|convert|start|begin|open|schedule|book|arrange|send|draft|write|compose|prepare|record|assign|reassign|complete|finish|cancel|reopen|reschedule|deactivate|activate)\b/i.test(source)
  );
}
  ...
  const actions = safePlan.actions.map((action) => {
    ...
    if (action.type === "create_project_from_customer") {
      ...
      if (!data.quote_id && customer?.id) {
        ...
        const preferred =
          customerQuotes.find((quote) => ["accepted", "approved"].includes(normalise(quote?.status))) ||
          customerQuotes[0] ||
          null;

        if (preferred?.id) data.quote_id = preferred.id;
      }
    }
  ...
  if (wantsStartProject && matchedCustomer?.id) {
    ...
    if (preferredQuote?.id) {
      actions.push({
        ...
        reason: `Create a project from ${preferredQuote.quote_number || "the matched quote"}.`,
        data: {
      ...
      return {
        mode: "analysis",
        summary: `${recordDisplayName(matchedCustomer)} does not have a quote available to start a project. Create a quote with an amount first, then I can start the project from that quote.`,
        actions: [],
      };
    }
  }
...
export function confirmationForPlan(plan) {
  ...
  const required = actions.length > 1 || actions.some((action) => {
    if (["convert_lead", "create_quote", "create_proposal", "create_project_from_customer", "create_invoice_from_quote", "record_invoice_payment", "mark_invoice_paid", "send_email"].includes(action.type)) return true;
...
async function createProjectFromCustomer({
  ...
  const service = clean(quote.service);
  const accountName = company || customerName || clean(quote.client) || "Customer";
  const projectName = service ? `${accountName} - ${service}` : `${accountName} Project`;
  ...
  const { data: project, error: projectError } = await supabase
    ...
    .insert([{
      ...
      description: service || `Delivery project for ${company || customerName || clean(quote.client) || "customer"}`,
