...
function extractExplicitEmailFromPrompt(prompt) {
...
function uniqueNonEmptyEmails(values) {
  return [
    ...new Set(
      (values || [])
...
export async function planRequest({ openai, prompt, conversation, businessData, employees, timezone, currentTime, localTime, forceActions = false }) {
  ...
  // =====================================================
  // DIRECT INVOICE PAYMENT SAFETY GATE
  //
  // Invalid payments must be stopped before an action plan is created.
  // This prevents Confirm & Run from appearing for a fully-paid invoice or
  // for a payment that exceeds the known outstanding balance.
  // =====================================================

  const directPaymentRequest =
    isClearActionInstruction(prompt) &&
    /\b(?:record|add|log)\b[\s\S]{0,25}\bpayment\b/i.test(
      String(prompt || "")
    );
  ...
  if (directPaymentRequest) {
    ...
    if (paymentInvoice?.id) {
      ...
      const hasPaymentSummary =
        Boolean(
          paymentInvoice.payment_summary &&
      ...
      const outstanding =
        hasPaymentSummary
          ? moneyValue(
              paymentInvoice.payment_summary.outstanding
            )
          : null;

      if (
        normalise(paymentInvoice.status) === "paid" ||
        (hasPaymentSummary && outstanding <= 0)
      ) {
        return sanitisePlan({
          mode: "analysis",
          summary:
            `${paymentInvoice.invoice_number || "This invoice"} is already paid in full. No additional payment can be recorded.`,
          actions: [],
        });
      }
      ...
      if (
        ...
        return sanitisePlan({
          mode: "analysis",
          summary:
            `The requested payment of £${requestedAmount.toFixed(2)} exceeds the outstanding balance of £${outstanding.toFixed(2)} for ${paymentInvoice.invoice_number || "this invoice"}.`,
          actions: [],
        });
      }
    }
  ...
  // =====================================================
  // DIRECT QUOTE ACCEPTANCE WORKFLOW
  // =====================================================

  const directAcceptedQuote =
    bestQuoteMatch(
      prompt,
      businessData
    );

  const directQuoteAcceptedRequest =
    isClearActionInstruction(prompt) &&
    directAcceptedQuote?.id &&
    (
      /\b(?:mark|set|change|update|accept)\b[\s\S]{0,45}\b(?:quote|snq-|q-)\b/i.test(
        String(prompt || "")
      ) ||
      /\b(?:quote|snq-|q-)\b[\s\S]{0,45}\baccepted\b/i.test(
        String(prompt || "")
      )
    ) &&
    /\baccepted?\b/i.test(
  ...
  if (directQuoteAcceptedRequest) {
    if (
      normalise(
        directAcceptedQuote.status
      ) === "accepted"
    ) {
      return sanitisePlan({
        mode: "analysis",
        summary:
          `${directAcceptedQuote.quote_number || "This quote"} is already Accepted.`,
        actions: [],
      });
    }
    ...
    return sanitisePlan({
      mode: "actions",
      summary:
        `Accepting quote ${directAcceptedQuote.quote_number || ""}.`,
      actions: [
        {
          type:
            "convert_quote_to_customer",
          label:
            `Accept ${directAcceptedQuote.quote_number || "quote"}`,
          reason:
            "Accept the explicitly named quote and ensure it is linked to its customer.",
          data: {
            record_id:
              directAcceptedQuote.id,
          },
  ...
  const plannedInvoiceSend =
    safePlan.actions.find(
      (action) =>
        action.type ===
        "send_invoice"
    );

  if (plannedInvoiceSend) {
    const sendInvoice =
      bestInvoiceMatch(
        prompt,
        businessData
      ...
      byId(
        ...
        plannedInvoiceSend.data?.record_id
      );
    ...
    if (sendInvoice?.id) {
      const customer =
        (businessData?.customers || []).find(
          (item) =>
            String(item?.id || "") ===
            String(sendInvoice.customer_id || "")
        ) ||
        null;

      const quote =
        (businessData?.quotes || []).find(
          (item) =>
            String(item?.id || "") ===
            String(sendInvoice.quote_id || "")
        ) ||
        null;

      const candidateEmails =
        uniqueNonEmptyEmails([
          customer?.email,
          quote?.email,
          sendInvoice.email,
        ]);
      ...
      const explicitEmail =
        extractExplicitEmailFromPrompt(
          prompt
        );
      ...
      if (
        ...
        return {
          mode:
            "analysis",
          summary:
            `${explicitEmail} is not a stored recipient for ${sendInvoice.invoice_number || "this invoice"}. Update the customer/quote first or use one of the stored recipient addresses.`,
          actions: [],
        };
      }
      ...
      if (
        ...
        return {
          mode:
            "analysis",
          summary:
            `${sendInvoice.invoice_number || "This invoice"} has conflicting stored recipient emails: ${candidateEmails.join(" and ")}. Please specify the exact email address to use before I send it.`,
          actions: [],
        };
      }
    }
...
- SAFETY: If an invoice's linked customer and quote contain different valid recipient emails, do not choose one silently. Ask the user to specify the exact stored email address.
...
function resolveRecordByText(collection, text, fields) {
    ...
    (collection || [])
      .map((record) => {
        ...
        for (const field of fields) {
          ...
          /*
           * Exact identifiers/emails/numbers are the strongest match.
           * This prevents a weaker company/name match from overriding an
           * explicitly supplied SNQ/SNI/SNP number or email address.
           */
  ...
  /*
   * Safety: if more than one distinct record has the same best score,
...
export function resolvePlanAgainstBusinessData({ plan, prompt, conversation, businessData, employees, timezone }) {
  ...
  const explicitPaymentAction =
    safePlan.actions.find(
      (action) =>
        action.type === "record_invoice_payment"
    );

  if (explicitPaymentAction) {
    const paymentInvoice =
      bestInvoiceMatch(
        prompt,
        businessData
      ...
      byId(
        businessData?.invoices,
        explicitPaymentAction.data?.record_id
      );

    if (paymentInvoice?.id) {
      const amount =
        moneyValue(
          explicitPaymentAction.data?.amount
        ) ||
        extractMoneyFromPrompt(
          prompt
        );
      ...
      if (
        ...
        return {
          mode: "analysis",
          summary:
            `The requested payment of £${amount.toFixed(2)} exceeds the outstanding balance of £${outstanding.toFixed(2)} for ${paymentInvoice.invoice_number || "this invoice"}.`,
          actions: [],
        };
      }
    }
    ...
    if (["update_invoice", "send_invoice", "cancel_invoice", "record_invoice_payment", "mark_invoice_paid"].includes(action.type)) {
      ...
      if (action.type === "send_invoice" && invoice) {
        ...
        if (
          ...
          /*
           * Conflict is stopped by the central guard above. Keep the action
           * unresolved here rather than trusting a stale planner recipient.
           */
          data.to =
            "";
        }
...
export function buildDeterministicPlan({ prompt, conversation, businessData, timezone }) {
    ...
    if (
      ...
      return {
        mode:
          "analysis",
        summary:
          `${explicitEmail} is not a stored recipient for ${invoice.invoice_number || "this invoice"}. Update the customer/quote first or specify one of the stored addresses.`,
        actions: [],
      };
    }
    ...
    if (
      ...
      return {
        mode:
          "analysis",
        summary:
          `${invoice.invoice_number || "This invoice"} has conflicting stored recipient emails: ${candidateEmails.join(" and ")}. Please specify the exact email address to use before I send it.`,
        actions: [],
      };
    }
  ...
  if (invoice?.id && wantsRecordPayment) {
    ...
    const outstanding =
      hasPaymentSummary
        ? moneyValue(
            invoice.payment_summary.outstanding
          )
        : null;

    if (
      normalise(invoice.status) === "paid" ||
      (hasPaymentSummary && outstanding <= 0)
    ) {
      return {
        mode: "analysis",
        summary: `${invoice.invoice_number || "This invoice"} is already paid in full. No additional payment can be recorded.`,
        actions: [],
      };
    }
    ...
    if (amount <= 0) {
      return {
        mode: "analysis",
        summary: "Payment amount must be greater than zero.",
        actions: [],
      };
    }
    ...
    if (
      ...
      return {
        mode: "analysis",
        summary: `The requested payment of £${amount.toFixed(2)} exceeds the outstanding balance of £${outstanding.toFixed(2)} for ${invoice.invoice_number || "this invoice"}.`,
        actions: [],
      };
    }
...
async function sendInvoiceRecord({ access, invoice, businessData, data }) {
  ...
  if (
    ...
    throw new Error(
      "The requested invoice recipient is not linked to this invoice's customer or quote."
    );
  }
  ...
  if (
    ...
    throw new Error(
      `This invoice has conflicting stored recipient emails (${candidateEmails.join(" and ")}). Please specify the exact email address before sending.`
    );
  }
