...
export function confirmationForPlan(plan) {
  ...
  const required = actions.length > 1 || actions.some((action) => {
    if (["convert_lead", "create_project_from_customer", "create_invoice_from_quote", "record_invoice_payment", "mark_invoice_paid", "send_email"].includes(action.type)) return true;
