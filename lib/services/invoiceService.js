...
export async function markInvoiceAsPaid({
  ...
  let matchedInvoice = findMatchingRecord(prompt, invoices, [
    "invoice_number",
    "client",
  ]);

  /*
   * The invoices table does not currently store the contact person's name.
   * Therefore, when the user says something like:
   *
   * "Mark Robert invoice as paid"
   *
   * we first find Robert's quote using the quote contact field, then locate
   * the invoice through the matching quote_id.
   */
  if (!matchedInvoice) {
